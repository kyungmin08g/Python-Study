...
# 함수 호출하여 동작 실행
hello_frist() # Hello, Python!

# 문제. 구구단 함수
def times_tables(num: int) -> None: # 반환 값이 없는 함수는 type hint가 없어도 됩니다.
    for j in range(1, 10):
        print(f"{num} x {j} = {num * j}") # 여기서 사용하는 num은 매개변수(parameter)

# 구구단 함수 호출
times_tables(2) # 여기서는 인수(arguments)
...
print(result_first) # 53

# [ print(result)랑 return result의 차이점 ] 중요 !!
# print(result)
def f2(x) -> None:
    a, b = 6, 2 # 변수 선언
    result = a * x + b # 최종 계산
...
result_second = f2(16)
print(result_second) # None (출력만 하고 반환 값이 없으니 None으로 대입됩니다.)
...
print(result_fourth) # 12: True, 그 외: False
